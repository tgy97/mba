...
class Reconstruction(object):
    def __init__(self, args):
        ...
        # initialize model
        self.use_conditional = args.use_conditional
        self.use_my_reconstruct = args.use_my_reconstruct
        if self.use_conditional:
            if self.use_my_reconstruct:
                self.model = my_ReconstructionNet(args)
            else:
                self.model = Conditional_ReconstructionNet(args)
        else:
    ...
    def train_epoch(self, epoch):
        ...
        for iter, (pts, label) in enumerate(self.train_loader):
            ...
            if self.use_conditional:
                output, f  = self.model(pts,label)
            else:
                output, f = self.model(pts)
