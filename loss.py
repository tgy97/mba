...
class KNN_self(nn.Module):
    def __init__(self):
        super(KNN_self, self).__init__()
        self.use_cuda = torch.cuda.is_available()
    ...
    def forward(self, preds, gts):
        P = self.batch_pairwise_dist(gts, preds)
        mins, _ = torch.topk(P, k = 3, dim = 2,largest = False) # 5, 3   // the second better  2   3           3
        mins = mins.mean(2)

        maxs, _ = torch.topk(mins,k =30,dim = 1) # 10 ,20                                     30  30           50
        loss = torch.sum(maxs)
        return loss
