...
def save_(dataset,item,gen,path,target = None):
    if target == None:
        n = 5
        l = torch.range(0,n-1).to(torch.int64)
    else:
        target = np.array(target)
        n = target.shape[0]
        l = torch.Tensor(target).to(torch.int64)
    pts, lb, mmm = dataset[item]

    output, _ = gen(pts.view(1, 2048, 3).repeat(n,1,1), l)
    new_path = path+str(item)
    os.makedirs(new_path, exist_ok=True)
    for i in range(n):
        t = output[i].detach().numpy()
        np.save(new_path+'/r_{}.npy'.format(i),t)
    np.save(new_path+'/o.npy'.format(i),pts.view(1, 2048, 3).squeeze().numpy())
...
def visualize(args):
    ...
    dataset = Dataset(root=args.dataset_root, dataset_name=args.dataset, 
                        num_points=args.num_points, split=args.split, load_name=True, use_origin_data = False)
    ...
    if args.use_conditional:
        if args.use_my_reconstruct:
            model = my_ReconstructionNet(args)
        else:
            model = Conditional_ReconstructionNet(args)
    else:
        model = ReconstructionNet(args)
    ...
    if one_model:
        ...
        if args.use_conditional:
            lb[:] = args.condition_class
            reconstructed_pl, _ = model(pts.view(1, 2048, 3).repeat(2,1,1),lb.repeat(2,1))
        else:
            reconstructed_pl, _ = model(pts.view(1, 2048, 3))
        ...
        r = os.path.join('mitsuba', experiment_id, args.dataset, args.split + str(args.item))
        r_name = '/r_{}.npy'.format(args.condition_class) if args.use_conditional else '/r.npy'
        save_path = os.path.join(r + r_name)


        r = os.path.join('mitsuba', experiment_id, args.dataset, args.split)
        save_(dataset,6778,model,r)
        embed()
        np.save(save_path,reconstructed_pl[0].detach().numpy())
        save_path = os.path.join(r + '/o.npy')
        np.save(save_path,pts.detach().numpy())
    else:
...
if __name__ == '__main__':  
    ...
    parser.add_argument('--use_conditional', action='store_true',
                        help='s')
    parser.add_argument('--condition_class', type=int, default=None, metavar='N',
                        help='c')   
